...
vector <int> snt_yellow_positions(10);
vector <int> gk_yellow_positions(10);
...
void get_ball_and_arm_positions(uint16_t* ball_pos_x, uint16_t* ball_pos_y, uint16_t* arm_human_gk_position, uint16_t* arm_human_snt_position)
{
    ...
    cv::inRange(frmHsv, cv::Scalar(20, 0, 0),
                cv::Scalar(179 , 255, 255), rangeRes_red_arm);
...
void get_ball_and_arm_positions2(uint16_t* ball_pos_x, uint16_t* ball_pos_y, uint16_t* arm_human_gk_position, uint16_t* arm_human_snt_position, uint16_t* arm_robot_snt_position)
{
    ...
    cv::Mat rangeRes_ball = cv::Mat::zeros(frame.size(), CV_8UC1);
    cv::Mat rangeRes_red_arm = cv::Mat::zeros(frame.size(), CV_8UC1);
    cv::Mat rangeRes_yellow_arm = cv::Mat::zeros(frame.size(), CV_8UC1);
    ...
    cv::inRange(frmHsv, cv::Scalar(20, 60, 70),
                cv::Scalar(40 , 255, 255), rangeRes_yellow_arm);
    // <<<<< Color Thresholding
    ...
    cv::erode(rangeRes_yellow_arm, rangeRes_yellow_arm, cv::Mat(), cv::Point(-1, -1), 2);
    cv::dilate(rangeRes_yellow_arm, rangeRes_yellow_arm, cv::Mat(), cv::Point(-1, -1), 2);
    // <<<<< Improving the result

    // >>>>> Contours detection
    vector<vector<cv::Point> > contours_for_ball;
    vector<vector<cv::Point> > contours_for_red_arm;
    vector<vector<cv::Point> > contours_for_yellow_arm;

    cv::findContours(rangeRes_ball, contours_for_ball, cv::RETR_EXTERNAL,
                     cv::CHAIN_APPROX_NONE);
    cv::findContours(rangeRes_red_arm, contours_for_red_arm, cv::RETR_EXTERNAL,
                     cv::CHAIN_APPROX_NONE);
    cv::findContours(rangeRes_yellow_arm, contours_for_yellow_arm, cv::RETR_EXTERNAL,
                     cv::CHAIN_APPROX_NONE);
    ...
    vector<vector<cv::Point>> balls;
    vector<vector<cv::Point>> red_players;
    vector<vector<cv::Point>> yellow_players;
    vector<cv::Rect> ballsBox;
    vector<cv::Rect> red_player_boxes;
    vector<cv::Rect> yellow_player_boxes;
    ...
    // Yellow arm detection drawing.
    for (size_t i = 0; i < contours_for_yellow_arm.size(); i++)
    {
        cv::Rect bBox;
        bBox = cv::boundingRect(contours_for_yellow_arm[i]);

        float ratio = (float) bBox.height / (float) bBox.width;
        yellow_players.push_back(contours_for_yellow_arm[i]);
        yellow_player_boxes.push_back(bBox);
    }
    ...
    // >>>>> Yellow Arm Detection result
    for (size_t i = 0; i < yellow_players.size(); i++)
    {
        cv::Point center;
        center.x = yellow_player_boxes[i].x + yellow_player_boxes[i].width / 2;
        center.y = yellow_player_boxes[i].y + yellow_player_boxes[i].height / 2;
    ...
    *ball_pos_y = 435 - (calculated_y_pos_ball - 45);
    ...
    if(snt_yellow_positions.size() == 3)
    {
        ...
        *arm_robot_snt_position = 435 - (calculated_y_pos_robot_snt - 45);
        last_pos_robot_snt = *arm_robot_snt_position;
    }
    else
    {
        *arm_robot_snt_position = last_pos_robot_snt;
    }
